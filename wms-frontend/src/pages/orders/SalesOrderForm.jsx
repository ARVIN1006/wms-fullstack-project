...
const schema = yup.object().shape({
  customer: yup.object().nullable().required("Customer wajib dipilih."),
  notes: yup.string(),
  items: yup
    .array()
    .of(
...
function SalesOrderForm() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) =>
        setCustomers(
          res.data.customers.map((c) => ({ value: c.id, label: c.name }))
        )
      )
      .catch((err) => toast.error("Gagal muat customers"));
  }, []);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.msg || "Gagal membuat SO");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-600 mb-6">
