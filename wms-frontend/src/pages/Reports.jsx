...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
    ...
    const renderPageButton = (page, label, customKey) => (
        <button
            // Pastikan menggunakan customKey yang unik untuk tombol navigasi
            key={customKey} 
            onClick={() => onPageChange(page)}
            // Nonaktifkan jika halaman yang dituju sama dengan halaman saat ini
            disabled={page === currentPage} 
            className={`px-3 py-1 mx-1 rounded-md text-sm transition ${
                page === currentPage
    ...
    return (
        <div className="flex justify-center items-center space-x-2 mt-6">
            ...
            {/* Tombol ke halaman pertama (key: 'first') */}
            {renderPageButton(1, '<<', 'first')} 
            
            {/* Tombol ke halaman sebelumnya (key: 'prev') */}
            {renderPageButton(Math.max(1, currentPage - 1), '<', 'prev')} 

            <span className="text-sm font-medium text-gray-700">
                Halaman {currentPage} dari {totalPages}
            </span>

            {/* Tombol ke halaman berikutnya (key: 'next') */}
            {renderPageButton(Math.min(totalPages, currentPage + 1), '>', 'next')}
            
            {/* Tombol ke halaman terakhir (key: 'last') */}
...
function Reports() {
    ...
    const LIMIT = 20; // Batas data per halaman
    ...
    useEffect(() => {
        async function fetchMasterAndReports() {
            ...
            try {
                ...
                // Muat Laporan Awal
                fetchReports(1); // Muat halaman pertama
            } catch (err) {
    ...
    async function fetchReports(page = currentPage) {
        try {
            ...
            // Perbarui state reports dan pagination
            const fetchedData = response.data;
            setReports(fetchedData.reports || []);
            setCurrentPage(fetchedData.currentPage || 1);
    ...
    const handleFilterSubmit = (e) => {
        e.preventDefault();
        setCurrentPage(1); // Selalu kembali ke halaman 1 saat filter baru diterapkan
        fetchReports(1); 
    }
    
    // Handler saat tombol halaman diklik
    ...
    return (
        <div className="p-6 bg-white shadow-lg rounded-lg">
            ...
            {/* Informasi Total dan Tombol Ekspor */}
            <div className="flex justify-between items-center mb-6">
                <p className="text-sm font-medium text-gray-600">Total Transaksi Ditemukan: **{loading ? '...' : totalCount.toLocaleString('id-ID')}**</p>
                <ExportButton 
                    data={getExportData()} 
                    headers={csvHeaders} 
                    filename={`Laporan_WMS_Page_${currentPage}_${new Date().toISOString().slice(0, 10)}.csv`}
                >
                    Unduh Halaman Ini (CSV)
                </ExportButton>
            </div>
