...
function MovementReport() {
  ...
  return (
    <div className="p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">📊 Laporan Perpindahan Barang</h1>
      ...
      <div className="flex justify-end items-center mb-6">
        <ExportButton 
            data={getExportData()} 
            headers={csvHeaders} 
            filename={`Laporan_Perpindahan_${new Date().toISOString().slice(0, 10)}.csv`}
        >
            Unduh Laporan (CSV)
        </ExportButton>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {reports.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{item.product_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold">{item.quantity}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600 font-semibold">{item.from_location_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">{item.to_location_name}</td>
      ...
      {reports.length === 0 && !loading && (
          <p className='text-gray-500 mt-4'>Tidak ada data pergerakan tercatat.</p>
      )}
    </div>
  );
}
