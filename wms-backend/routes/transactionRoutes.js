...
router.post('/in', auth, authorize(['admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    const operator_id = req.user.id; // Operator ID dari token
    ...
    for (const item of items) {
      const { product_id, location_id, quantity, stock_status_id, purchase_price } = item; // <-- BARU: Ambil Status & Harga Beli

      // 2a. Validasi Dasar
      if (quantity <= 0 || !product_id || !location_id || !stock_status_id) {
          throw new Error("Item baris wajib memiliki Produk, Lokasi, Status, dan Jumlah.");
      }

      // 2b. Menyimpan detail di transaction_items
      await client.query(
        'INSERT INTO transaction_items (transaction_id, product_id, location_id, quantity, stock_status_id) VALUES ($1, $2, $3, $4, $5)',
        [transactionId, product_id, location_id, quantity, stock_status_id] // <-- Simpan Status Stok
      );
...
router.post('/out', auth, authorize(['admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    // Ambil customer_id (baru) dan items
    ...
    // 2. Proses Setiap Item Barang
    for (const item of items) {
      const { product_id, location_id, quantity, stock_status_id, selling_price } = item; // <-- BARU: Ambil Status & Harga Jual
      ...
      if (currentStock < quantity) {
        throw new Error(`Stok tidak cukup di lokasi asal. Sisa: ${currentStock}, Diminta: ${quantity}`);
      }
