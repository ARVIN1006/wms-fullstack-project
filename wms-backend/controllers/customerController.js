...
exports.getCustomers = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      whereClauses.push(
        `(name ILIKE $${paramIndex} OR contact_person ILIKE $${paramIndex})`
      );
    }
    ...
    // Query B: Ambil data (Perhatikan penambahan parameter index)
    ...
    const dataQuery = `
      SELECT * FROM customers ${whereString} 
      ORDER BY name ASC 
      LIMIT $${limitIndex} 
      OFFSET $${offsetIndex}
    `;
    ...
    console.error("ERROR IN GET /API/CUSTOMERS:", err.message);
    res.status(500).send("Server Error: Gagal memuat data pelanggan.");
  }
};
...
exports.createCustomer = async (req, res) => {
  try {
    ...
    if (!name)
      return res.status(400).json({ msg: "Nama customer wajib diisi." });
