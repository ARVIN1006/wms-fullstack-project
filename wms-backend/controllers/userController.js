...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    const userToEditRes = await client.query(
    ...
    if (originalRole === "admin" && role && role !== "admin") {
      ...
      if (adminCount <= 1) {
        ...
        return res
          .status(403)
          .json({
            msg: "Gagal. Anda adalah Admin terakhir, role Anda tidak bisa diturunkan.",
          });
      }
    }
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (req.user.id === parseInt(id, 10)) {
      return res
        .status(400)
        .json({ msg: "Anda tidak bisa menghapus akun Anda sendiri." });
    }
