...
async function superSeeder() {
  ...
  try {
    ...
    console.log('Transaksi dimulai. Menghapus tabel lama (Prioritas Dependensi)...');
    ...
    // STATUSES & CATEGORIES
    const statusRes = await client.query("INSERT INTO stock_statuses (name) VALUES ('Good'), ('Damaged'), ('Expired') RETURNING id");
    const statusGoodId = statusRes.rows[0].id;
    const statusDamagedId = statusRes.rows[1].id;
    
    const categoryRes = await client.query("INSERT INTO categories (name) VALUES ('Electronics'), ('Office Supplies') RETURNING id");
    ...
    const locRes = await client.query("INSERT INTO locations (name) VALUES ('RAK A1 (Elektronik)'), ('RAK B2 (Kantor)'), ('QA AREA'), ('GUDANG TERTUTUP') RETURNING id");
    const locIdA1 = locRes.rows[0].id;
    const locIdB2 = locRes.rows[1].id;
    const locIdQA = locRes.rows[2].id;

    const supplierRes = await client.query("INSERT INTO suppliers (name) VALUES ('PT. Supplier Emas'), ('CV. Distributor Cepat'), ('Toko Lokal Murah') RETURNING id");
    const customerRes = await client.query("INSERT INTO customers (name) VALUES ('Toko Jaya Abadi'), ('Pelanggan Online'), ('CV. Ritel Besar') RETURNING id");
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      const sku = isElectronic ? `ELEC-${i.toString().padStart(3, '0')}` : `OFFICE-${(i-25).toString().padStart(3, '0')}`;
      const name = isElectronic ? `Headset Gaming V${i}` : `Kertas A4 HVS ${i-25} rim`;
      ...
      const res = await client.query(
        'INSERT INTO products (sku, name, unit, category_id, purchase_price, selling_price, main_supplier_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id',
        [sku, name, 'pcs', catId, price, selling, suppId]
      );
    ...
    for (let i = 0; i < 100; i++) {
        ...
        const transRes = await client.query(
            ...
            [type, `${type} Order #${i}`, supplierId, customerId, operatorId, date, new Date(date.getTime() + durationMin * 60000)]
        );
        ...
        if (type === 'OUT') {
            const stockCheck = await client.query('SELECT quantity FROM stock_levels WHERE product_id = $1 AND location_id = $2', [productId, locationId]);
    ...
    await client.query(
        ...
        [productIds[0], locIdA1, locIdQA, 5, 'QA/Pengecekan Kualitas', adminUserId]
    );
    // Update stok untuk perpindahan
    await client.query('UPDATE stock_levels SET quantity = quantity - 5 WHERE product_id = $1 AND location_id = $2', [productIds[0], locIdA1]);
    await client.query('INSERT INTO stock_levels (product_id, location_id, quantity) VALUES ($1, $2, $3) ON CONFLICT (product_id, location_id) DO UPDATE SET quantity = stock_levels.quantity + EXCLUDED.quantity', [productIds[0], locIdQA, 5]);
